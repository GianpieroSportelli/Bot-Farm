...
def get_tokenier(url="https://tfhub.dev/tensorflow/bert_multi_cased_L-12_H-768_A-12/1", fine_tune=True,
                 do_lower_case=True):
...
class Bert(PrunableLayer, Layer):
    def __init__(self, input_shape=(1), output_representation='cls_output', fine_tuning=True,
                 url="https://tfhub.dev/tensorflow/bert_multi_cased_L-12_H-768_A-12/1", **kwargs):
        super(Bert, self).__init__(**kwargs)
        self.output_representation = output_representation
        self.bert = hub.KerasLayer(url, trainable=fine_tuning)
        super(Bert, self).build(input_shape)
        for w in self.bert.trainable_variables:
            if "adam" in w.name:
                tf.get_logger().info(w)
        # self.add_weight(self.bert.get_weights())
